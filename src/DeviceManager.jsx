  ...
  return <section className="device-manager">
    <div className="device-manager__heading">
      <div>
        ...
        <p>{language === "de"
          ? `Maximal ${data.limit} registrierte Geräte. Entferne Geräte, die du nicht mehr verwendest.`
          : `Up to ${data.limit} registered devices. Remove devices you no longer use.`}</p>
      </div>
    ...
    <div className="device-manager__groups">
      ...
      <div>
        ...
        {data.devices.filter((device) => device.status === "ACTIVE").length
          ? data.devices.filter((device) => device.status === "ACTIVE").map((device) => <article className="device-card" key={device.id}>
            <div><strong>{device.displayName || (language === "de" ? "Persönliches Gerät" : "Personal device")}{device.current ? ` · ${language === "de" ? "dieses Gerät" : "this device"}` : ""}</strong>
              <span>{language === "de" ? "Zuletzt aktiv" : "Last active"}: {when(device.lastSeenAt, language)}</span></div>
